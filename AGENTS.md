...
- `utilities.nix` — CLI tools (ripgrep, bat, eza, fd, jq, gh, `Codex`, etc.) plus container tooling (`docker-client`, `docker-compose`, `podman`).
...
- **Unfree packages** must be whitelisted by name in `flake.nix`'s `allowedUnfree` list (currently only `Codex`).
